      ...
      try {
        ...
        if (executedTool) {
          ...
          if (executedTool === 'consultar_catalogo') {
            ...
            try {
              const filteredTools = botTools.filter((t: any) => t.function?.name !== 'consultar_catalogo');
              const secondPassBody: any = {
                model: 'gpt-4o',
              ...
              if (filteredTools.length > 0) {
                secondPassBody.tools = filteredTools;
                secondPassBody.tool_choice = 'auto';
              }
              ...
              if (secondRes.ok) {
                ...
                console.error('[RAG SECOND PASS] Error HTTP de OpenAI:', secondRes.status, errorText);
                aiResponse = 'Lo siento, no pude procesar el catálogo en este momento.';
              }
            } catch (ragErr) {
